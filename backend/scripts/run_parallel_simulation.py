...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout stream to UTF-8 (fix console encoding issues)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Quote post: add original post content, author, and quote comment
            ...
            # Get the quote content of the quoted post (quote_content)
            if new_post_id:
                cursor.execute("""
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If a boost LLM is configured, parallel simulations can use different API providers for different platforms, increasing concurrency.
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if available, otherwise falls back to standard configuration)
...
def setup_signal_handlers(loop=None):
    ...
    def signal_handler(signum, frame):
        ...
        # Don't call sys.exit() directly; let the asyncio loop exit gracefully and clean up resources
