...
def optimize_interview_prompt(prompt: str) -> str:
    ...
    if not prompt:
        return prompt
    # Avoid adding the prefix if it's already present
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required files (scripts excluded; they reside in backend/scripts/)
    required_files = [
        "state.json",
...
def prepare_simulation():
    ...
    4. LLM intelligently generates simulation configuration (with retry)
    ...
    try:
        ...
        # This lets the frontend immediately get the expected total Agent count after calling prepare
        try:
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates over the reports directory, finds reports matching the simulation_id,
    and returns the newest one (sorted by created_at).
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    try:
        ...
        # Extract key summary statistics if config exists
        if config:
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it will be stopped and run logs cleaned up
