...
class OasisProfileGenerator:
    ...
    2. Generate highly detailed personas (including basic info, career experience, personality traits, social media behavior, etc.)
    3. Distinguish between individual entities and abstract group entities
    """
    ...
    def _generate_profile_with_llm(
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Check if truncated (finish_reason not 'stop')
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Fix truncated JSON (output cut by max_tokens limit)"""
        import re
        ...
        # Simple check: if last char after final quote is not comma or closing bracket, string may be truncated
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in social media user profile generation. Generate detailed, realistic personas for public opinion simulation, maximizing fidelity to existing real-world information. You must return valid JSON format; all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, maximizing fidelity to existing real-world information.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Distinctive traits (catchphrases, special experiences, personal interests)
   - Personal memory (important part of persona; describe this individual's connection to events and their existing actions/reactions in the events)
3. age: Age as integer (must be integer)
...
- persona must be one coherent text description
- Use English (except gender field must use English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institution/group entity, maximizing fidelity to existing real-world information.
...
2. persona: Detailed account profile description (2000 words of plain text), must include:
   ...
   - Institutional memory (important part of institution persona; describe this institution's connection to events and its existing actions/reactions in the events)
3. age: Fixed value 30 (virtual age for institution account)
4. gender: Fixed value "other" (institution accounts use other for non-individual)
...
- persona must be one coherent text description, no newlines
- Use English (except gender field must use English "other")
- age must be integer 30, gender must be string "other"
- Institution account communication must match its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: Path for real-time write (if provided, write after each generation)
        ...
        # Helper for real-time file write
        def save_profiles_realtime():
            ...
            with lock:
                # Filter to generated profiles only
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Output to console only (avoid duplication, logger does not output full content)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save Profiles to file (select format by platform)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # Replace newlines (use space in CSV)
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Maps Chinese input (male, female, institution, other) to OASIS format
        gender_map = {
            "male": "male",      # male
            "female": "female",    # female
            "mechanism": "other",   # institution
            "other": "other",   # other
            # English already
            "male": "male",
            "female": "female",
