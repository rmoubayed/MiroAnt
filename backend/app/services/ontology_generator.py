...
- Each entity is an "account" or "actor" that can speak, interact, and disseminate information on social media
...
A. **Fallback types (must include, placed last 2 in the list)**:
   - `Person`: Fallback type for any individual. When a person does not belong to other more specific person types, use this.
   - `Organization`: Fallback type for any organization. When an organization does not belong to other more specific organization types, use this.
...
- The text will contain various people, such as "primary/secondary school teachers", "passersby", "a certain netizen"
- If there is no dedicated type to match, they should be classified as `Person`
