...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError("Simulation configuration does not exist, please call /prepare endpoint first")
        ...
        try:
            ...
            # Create main log file to avoid process blocking from stdout/stderr pipe buffer full
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess env vars to ensure UTF-8 encoding on Windows
            # This fixes issues when third-party libs (e.g. OASIS) read files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+, makes all open() use UTF-8 by default
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set working directory to simulation directory (database etc. will be created here)
            # Use start_new_session=True to create new process group, ensures all child processes can be terminated via os.killpg
            process = subprocess.Popen(
                cmd,
                cwd=sim_dir,
                stdout=main_log_file,
                stderr=subprocess.STDOUT,  # stderr also goes to same file
                text=True,
                encoding='utf-8',  # Explicit encoding
                bufsize=1,
                env=env,  # Pass env with UTF-8 settings
                start_new_session=True,  # Create new process group, ensures all related processes can be terminated on server shutdown
            )
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines which platforms are enabled by checking if corresponding actions.jsonl exists
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # With start_new_session=True, process group ID equals main process PID
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer record's platform, else use default
    ...
    def get_all_actions(
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Legacy format should have platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Note: Does not delete config file (simulation_config.json) and profile files
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean (avoid useless logs for empty processes)
    ...
    def register_cleanup(cls):
        ...
        Called when Flask app starts to ensure all simulation processes are cleaned on server shutdown
        """
        ...
        # If not debug mode, this env var is absent, still need to register
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: For dual-platform simulation, interview both platforms and return combined result
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: Simulation ID
            interviews: Interview list, each element has {"agent_id": int, "prompt": str, "platform": str(optional)}
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": Default Twitter platform only
                - "reddit": Default Reddit platform only
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation for graceful exit from wait-for-command mode
        ...
        try:
            ...
            # Timeout may mean environment is shutting down
            return {
                "success": True,
