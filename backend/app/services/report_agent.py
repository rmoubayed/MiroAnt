...
4. Supports chat with users, autonomously invoking retrieval tools during conversation
"""
...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Log section content generation complete (content only, not full section)"""
        self.log(
    ...
    def log_section_full_complete(
        ...
        Frontend should listen to this log to determine when a section is truly complete and get full content
        """
        self.log(
...
[Broad Search - Get full panorama view]
This tool is used to obtain the complete panorama of simulation results, especially suitable for understanding event evolution. It will:
1. Retrieve all related nodes and relationships
2. Distinguish between current valid facts and historical/expired facts
3. Help you understand how sentiment evolved
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports", with a "god's-eye view" of the simulation world—you can see every Agent's behavior, statements, and interactions in the simulation.

[Core concept]
We built a simulation world and injected specific "simulation requirements" as variables. The evolution results of the simulation world are predictions of what may happen in the future. What you are observing is not "experimental data" but "a rehearsal of the future".
...
[Report positioning]
- ✅ This is a simulation-based future prediction report revealing "if this happens, what the future will be like"
- ✅ Focus on prediction results: event trajectory, group reactions, emergent phenomena, potential risks
- ✅ Agent statements and actions in the simulation world are predictions of future human behavior
- ❌ Not analysis of current real-world status
- ❌ Not generic sentiment overview

[Section count limit]
- Minimum 2 sections, maximum 5 sections
- No sub-sections needed; each section directly contains full content
...
1. Under our conditions, what state did the future present?
...
❌ Do not write as analysis of current real-world status
...
3. [Language consistency - quoted content must be translated to report language]
   - Tool-returned content may contain English or mixed English-Chinese
   - If simulation requirements and source material are in Chinese, the report must be written entirely in Chinese
   - When quoting English or mixed content from tools, translate it to fluent Chinese before writing into the report
   - Preserve original meaning when translating, ensure natural expression
...
- ❌ Do not add section main title at content start
- ✅ Section titles are added automatically by the system; you only write pure body content
- ✅ Use **bold**, paragraph breaks, quotes, lists to organize content, but not headings

[Correct example]
```
This section analyzes the event's sentiment propagation. Through in-depth analysis of simulation data, we find...

**Initial breakout phase**

As the first scene of sentiment, the platform bore the core function of information release:

> "The platform contributed 68% of initial volume..."

**Emotion amplification phase**

Another platform further amplified event impact:
...
[Tool usage suggestion - mix different tools, do not use only one]
- insight_forge: Deep insight analysis, auto-decompose questions and retrieve facts and relationships multi-dimensionally
- panorama_search: Wide-angle panorama search, understand event full picture, timeline and evolution
...
The system will execute the tool and return results to you. You need not and cannot write tool results yourself.

Option B - Output final content:
When you have obtained enough information through tools, output section content starting with "Final Answer:".

⚠️ Strictly forbidden:
- Do not include both tool call and Final Answer in one reply
...
2. Quote extensively from source text to demonstrate simulation effects
...
4. [Quote format rule - must be standalone paragraphs]
   Quotes must be standalone paragraphs with a blank line before and after, not mixed in paragraphs:
   ...
   ❌ Wrong format:
   ```
   The response was deemed lacking. > "The response pattern..." This assessment reflects...
   ```
...
- ✅ Write body content directly, use **bold** instead of sub-section titles
...
class ReportAgent:
    ...
    # Valid tool name set, for validation when parsing bare JSON fallback
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM outputs bare JSON directly (without <tool_call> wrapper)
    ...
    def _generate_section_react(
        ...
        # Build user prompt - each completed section up to 4000 chars
        ...
        conflict_retries = 0  # Consecutive conflicts when tool call and Final Answer appear together
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None, break to force completion
                break
            ...
            if has_final_answer:
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = f" (these tools not yet used, consider trying them: {', '.join(unused_tools)})" if unused_tools else ""
                    messages.append({
                        "role": "user",
            ...
            if has_tool_calls:
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM attempted to call {len(tool_calls)} tools, executing only first: {call['name']}")
            ...
            # Directly use this content as final answer, no more empty loops
    ...
    def chat(
        ...
        Agent can autonomously call retrieval tools to answer questions during chat
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Line number to start reading (for incremental fetch, 0 = from start)
    ...
    def save_section(
        ...
        Called immediately after each section generation completes, for section-by-section output
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        2. Convert ### and lower-level headings to bold text
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if duplicate heading (heading with same content within 5 consecutive lines)
                is_duplicate = False
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Backward compatibility: check files stored directly under reports directory
