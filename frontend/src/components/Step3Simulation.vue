...
// Computed
// Display actions in chronological order (latest at the end, i.e. bottom)
...
// Format elapsed simulation time (calculated by round count and minutes per round)
...
// Elapsed simulation time for Twitter platform
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Elapsed simulation time for Reddit platform
...
const resetAllState = () => {
  ...
  stopPolling()  // Stop any previous polling that may still exist
}
...
const doStartSimulation = async () => {
  ...
  // Reset all state first to avoid carry-over from the previous simulation
  resetAllState()
...
// Track previous round for each platform to detect changes and emit logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes for each platform and emit logs
      ...
      // Detect whether simulation has completed (by runner_status or platform completion flags)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: backend runner_status might lag while platforms are already done
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform is involved, check if all enabled platforms are complete
  // Infer enabled platforms by actions_count (count > 0) or running/completed history
