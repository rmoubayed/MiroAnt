<template>
  ...
  >
    <!-- Background decoration: tech grid lines (show only when projects exist) -->
    ...
    <!-- Card container (show only when projects exist) -->
...
let pendingState = null  // Record the target state pending execution
...
const containerStyle = computed(() => {
  ...
  // Compute required height: rows * card height + (rows - 1) * gap + small bottom spacing
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count in current row to keep each row centered
    ...
    // Expand downward and increase spacing from the title
    ...
    // Collapsed state: fan-like stack
    const transition = 'transform 700ms cubic-bezier(0.23, 1, 0.32, 1), opacity 700ms cubic-bezier(0.23, 1, 0.32, 1), box-shadow 0.3s ease, border-color 0.3s ease'
    ...
    const x = offset * 35
    // Adjust start position: closer to title with proper spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state (always track latest target even during animation)
        ...
        // If animation is in progress, only record state and handle after animation ends
        if (isAnimating) return
        
        // No work needed when target state matches current state
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounced state switch to prevent fast flicker
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether pending state still needs execution (may be overridden)
          ...
          setTimeout(() => {
            ...
            // After animation, check whether a new pending state exists
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Execute after a short delay to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink viewport bottom so expand triggers after more scroll
...
// Watch route changes and reload data when returning to home
...
// If keep-alive is used, reload data on component activation
onActivated(() => {
  loadHistory()
})
...
/* Create a square grid with fixed spacing using CSS background patterns */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor at top-left; when height changes, expand only at bottom */
