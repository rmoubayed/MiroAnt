    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG short- and long-term memory updating in real time' : 'Updating in real time...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still being processed. Please refresh the graph manually later.</span>
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to idle; show completion hint
...
const renderGraph = () => {
  ...
  // Process edge data: count edges and index per node pair
  const edgePairCount = {}
  ...
  // Track current edge index within each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edge: add only one merged edge per node
    ...
    // Check if edge direction matches normalized ordering (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Compute curvature: spread multiple edges, keep single edge straight
    let curvature = 0
    if (totalCount > 1) {
      ...
      // This keeps edges distributed in one reference frame and avoids overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - dynamically adjust spacing based on edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance by edge count between the pair
      // Base distance is 150; add spacing as edge count increases
    ...
    // Add gravity toward center so disconnected clusters drift inward
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from the node's right side and loop back
    ...
    // Compute curve control point - dynamically based on edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset based on distance ratio for visible curves
    // More edges increase the offset ratio
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop label position: to the right of node
      return { x: sx + 70, y: sy }
    }
  ...
  // Link label backgrounds (white background improves readability)
  ...
  // Save refs for external visibility toggling
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position; do not restart simulation (separates click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect true drag start (movement beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart simulation only after true drag is detected
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only cool down simulation after an actual drag
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (no rotation for better readability)
