            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, section may still be in progress) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM updates, preserve button viewport position by adjusting scroll
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract stats - matches "Related Prediction Facts: X" format
    ...
    // Extract sub-questions - complete extraction with no count limit
    const subQSection = text.match(/### (?:Analyzed Sub-Questions|\u5206\u6790\u7684\u5B50\u95EE\u9898)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Extract key facts - complete extraction with no count limit
    ...
    // Extract relationship chains - complete extraction with no count limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - complete extraction with no count limit
    ...
    // Extract historical/expired facts - complete extraction with no count limit
    ...
    // Extract involved entities - complete extraction with no count limit
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interview subject selection rationale
    const reasonMatch = text.match(/### (?:Selection Rationale|\u91C7\u8BBF\u5BF9\u8C61\u9009\u62E9\u7406\u7531)\n([\s\S]*?)(?=\n---\n|\n### (?:Interview Transcript|\u91C7\u8BBF\u5B9E\u5F55))/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: - Select Parent_601 (index 0): ...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation line for rationale (exclude ending summary paragraphs)
          currentReason.push(line.trim())
        }
      }
      
      // Save final person's rationale
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (e.g., "Student", "Education Practitioner")
      ...
      if (qMatch) {
        ...
        // Split questions by numeric order
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Special handling when the first question begins with "1."
      ...
      if (answerMatch) {
        ...
        const twitterMatch = answerText.match(/(?:\[(?:Twitter Answer|Twitter Platform Answer)\]|[\u3010]Twitter\u5E73\u53F0\u56DE\u7B54[\u3011])\n?([\s\S]*?)(?=(?:\[(?:Reddit Answer|Reddit Platform Answer)\]|[\u3010]Reddit\u5E73\u53F0\u56DE\u7B54[\u3011])|$)/)
        const redditMatch = answerText.match(/(?:\[(?:Reddit Answer|Reddit Platform Answer)\]|[\u3010]Reddit\u5E73\u53F0\u56DE\u7B54[\u3011])\n?([\s\S]*?)$/)
        ...
        // Platform fallback logic (compatible with legacy single-platform format)
        if (!twitterMatch && redditMatch) {
          // Reddit-only answer; copy to default display only if non-placeholder
          ...
          // No platform markers (very old format): treat whole text as answer
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > \u201Ctext\u201D (CJK curly quotes)
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - complete extraction with no count limit
    ...
    // Try extracting edge information (if present)
    ...
    // Try extracting node information (if present)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-Questions Generated by Drift Query Analysis'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Maintain independent platform selection for each question-answer pair
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response from this platform)' || t === '(\u8BE5\u5E73\u53F0\u672A\u83B7\u5F97\u56DE\u590D)' || t === '[No Response]' || t === '[\u65E0\u56DE\u590D]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbering or only one index is found, return full text
      if (matches.length <= 1) {
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real (non-placeholder) and different answers
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch buttons (shown only when both have real answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 title (## xxx) since section title is already shown externally
  ...
  // Clean whitespace after opening list tags
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Clean whitespace before closing list tags
  ...
  // Process line breaks: blank lines -> paragraph split, single break -> <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean cases where <p><br> is immediately followed by block elements
  ...
  // Fix numbering in non-contiguous ordered lists: preserve incremental numbering
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll handling is unified in nextTick after the loop
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If completed, scroll to top; otherwise follow latest logs at bottom
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try extracting content after legacy Chinese final-answer marker
  const chineseFinalMatch = response.match(/\u6700\u7EC8\u7B54\u6848[:\uFF1A]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // If it starts with ## / # / >, it may already be markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and markdown-like, try removing thought text
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove leading "Thought:" reasoning text
