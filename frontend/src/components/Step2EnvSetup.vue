    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combine context and automatically invoke tools to extract entities and relationships from the knowledge graph, initialize simulated individuals, and assign unique behaviors and memories based on real-world seeds.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM sets parameters intelligently based on simulation goals and real-world seeds, including world time flow, recommendation algorithms, each individual's active windows, posting frequency, and event triggers.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generate initial activation events and trending topics based on narrative direction to guide the initial state of the simulation world.
          </p>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For a first run, it is strongly recommended to switch to Custom Mode and reduce simulation rounds so you can preview results faster and lower error risk. ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">Event Journey Overview</span>
...
// Log deduplication: track key info from the previous output
...
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null before config is generated
  }
  ...
  // Ensure max rounds are at least 40 (recommended) to avoid invalid slider range
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User-selected custom rounds: pass max_rounds parameter
...
const startPrepareSimulation = async () => {
  ...
  // Mark step one complete and start step two
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Immediately set expected total agent count from the prepare response
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Update only when API returns a valid value to avoid overwriting existing valid data
...
const loadPreparedData = async () => {
  ...
  // Fetch configuration via the realtime endpoint
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Configuration is still generating. Polling for updates...')
        startConfigPolling()
      }
    }
  } catch (err) {
