...
const handleNextStep = (params = {}) => {
  ...
  // Record simulation round settings
  if (params.maxRounds) {
  ...
  // Pass custom round count via query params if provided
  if (params.maxRounds) {
 ...
 * When users return from Step 3 to Step 2, assume they want to exit simulation.
 */
const checkAndStopRunningSimulation = async () => {
  if (!currentSimulationId.value) return
  
  try {
    // Check whether simulation environment is alive first
    ...
    // Failure to check environment status should not block the flow
