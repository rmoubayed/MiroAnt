...
// Read maxRounds from query at initialization so child component can use it immediately
...
const handleGoBack = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running, check whether process still needs stopping
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // Load simulation config to get minutes_per_round
      try {
...
const loadGraph = async (graphId) => {
  // While simulating, do not show fullscreen loading during auto-refresh to avoid flicker
...
onMounted(() => {
  ...
  // Record maxRounds config (already initialized from query parameter)
  if (maxRounds.value) {
